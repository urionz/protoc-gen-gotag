...
import (
	...
	"github.com/urionz/protoc-gen-gotag/tagger"
)
...
func (v *tagExtractor) VisitOneOf(o pgs.OneOf) (pgs.Visitor, error) {
	var ptags []*tagger.Tag
	ok, err := o.Extension(tagger.E_OneofTags, &ptags)
	if err != nil {
		return nil, err
	}
...
func (v *tagExtractor) VisitField(f pgs.Field) (pgs.Visitor, error) {
	var ptags []*tagger.Tag
	ok, err := f.Extension(tagger.E_Tags, &ptags)
	if err != nil {
		return nil, err
	}
	...
	newTags, err := structtag.Parse(tagStr)
	v.CheckErr(err)
